...
print(f"✅ Баннер: {'найден' if os.path.exists(BANNER_PATH) else '❌ не найден (будет текст)'}")
print(f"✅ ffmpeg: {FFMPEG_LOCATION or 'системный'}")
...
async def show_subscribe_screen(chat_id, context):
    ...
    try:
        ...
        print(f"❌ Ошибка show_subscribe_screen: {e}")
        await context.bot.send_message(chat_id=chat_id, text=caption,
            parse_mode="HTML", reply_markup=keyboard)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if not subscribed:
            await show_subscribe_screen(chat_id, context)
            return
        ...
        caption = (
            ...
            "В группе пиши в поле сообщения:\n"
        ...
        try:
            await update.message.reply_text("🎵 Lira Music запущен! Отправь название трека или ссылку.")
        except Exception:
            pass

# ──────────────────────────────────────────────
# Inline режим для групп
# ──────────────────────────────────────────────
async def inline_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not query_text:
        results = [
            InlineQueryResultArticle(
                id="hint",
                title="🎵 Lira Music — введи название трека",
    ...
    try:
        ...
        print(f"❌ Ошибка inline поиска: {e}")
        tracks = []
        ...
        results.append(
            InlineQueryResultArticle(
                ...
                input_message_content=InputTextMessageContent(
                    ...
                    f"⏳ Нажми кнопку ниже чтобы скачать\n\n"
    ...
    if data == "check_sub":
        ...
        if await is_subscribed(query.from_user.id, context):
            ...
            try:
                ...
                print(f"❌ Ошибка после подписки: {e}")
        else:
